...
import { Calendar, Download, ExternalLink, Check, Smartphone, Bell, X, Trash2, Sparkles } from 'lucide-react';
...
export default function CalendarSyncModal({ scheduleItems, userId, isOpen, onClose }: CalendarSyncModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-lg glass-card border border-sky-900/40 shadow-2xl overflow-hidden rounded-2xl">
        {/* Modal Header */}
        ...
        <div className="p-6 space-y-6 max-h-[75vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="p-4 rounded-xl bg-[#091727]/90 border border-sky-900/40 space-y-3">
            <div className="flex items-start space-x-3">
              <div className="p-2.5 rounded-lg bg-sky-500/20 text-sky-400 shrink-0 mt-0.5">
              ...
              <div className="space-y-1 flex-1">
                ...
                <p className="text-[11px] text-sky-200/70 font-normal">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-2">
              <button
                ...
                className={`w-full sm:flex-1 py-3 px-4 rounded-xl font-bold text-xs flex items-center justify-center space-x-2 transition-all shadow-md sky-bounce ${
                  downloaded
              ...
              <button
                ...
                className="w-full sm:w-auto py-3 px-3 rounded-xl font-semibold text-xs bg-red-500/10 hover:bg-red-500/20 text-red-300 border border-red-500/30 flex items-center justify-center space-x-1.5 transition-all sky-bounce"
            ...
            <div className="space-y-2 pt-1 max-h-48 overflow-y-auto pr-1">
              {sortedItems.map((item, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2.5 rounded-xl bg-[#081524]/90 border border-sky-900/40 text-xs"
                  >
                    <div className="truncate pr-2">
                      <span className="font-semibold text-sky-100 block truncate">{item.subject_name}</span>
                      <span className="text-[10px] text-sky-300/70 font-mono font-medium">
                    ...
                    <a
                      ...
                      className="inline-flex items-center space-x-1 px-3 py-1.5 rounded-lg bg-sky-500/20 hover:bg-sky-500/40 text-sky-300 font-semibold shrink-0 transition-all border border-sky-500/30 text-[11px] sky-bounce"
                    >
                      <span>Thêm</span>
          ...
          {/* Instructions Box */}
          <div className="p-3.5 bg-sky-500/10 border border-sky-500/20 rounded-xl text-sky-200 text-xs flex items-start space-x-2 font-normal">
            <Bell className="w-4 h-4 shrink-0 mt-0.5 text-sky-400" />
            <span>
        ...
        {/* Modal Footer */}
        <div className="p-4 bg-[#081524] border-t border-sky-900/40 text-right">
          <button
            onClick={onClose}
            className="px-5 py-2 rounded-xl bg-[#0f2136] hover:bg-sky-950/70 text-sky-200 text-xs font-semibold border border-sky-500/20 transition-all sky-bounce"
          >
            Đóng
          </button>
        </div>
